...
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as { workspaceId?: string; subject?: string; actor?: string } | null;
  ...
  await appendAuditEvent(store, { workspaceId, actor: body.actor || "system", action: "passport.published", payload: { passportId: passport.id, score: passport.score } });
