...
export async function POST(request: Request) {
  ...
  return withAuth(request, async (userId, address) => {
    ...
    await appendAuditEvent(store, { workspaceId, caseId: item.id, actor: userId, action: "queue.created", payload: { priority: queueItem.priority, assigned: Boolean(assignment) } });
